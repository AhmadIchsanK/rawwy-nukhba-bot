...
async def announce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in targets:
        try:
            ...
            async with pool.acquire() as conn: await conn.execute("INSERT INTO announcement_messages (announcement_id, chat_id, message_id) VALUES ($1, $2, $3)", a_id, t['chat_id'], m.message_id)
        except: pass
...
async def super_reset_req(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cb = f"sup_reset_{target}"
    kb = [[InlineKeyboardButton("⚠️ Wipe", callback_data=cb), InlineKeyboardButton("Cancel", callback_data="sup_cancel")]]
