...
async def auto_return_away(context):
    pool = context.bot_data.get('db_pool'); username = context.job.data['username']; uid = await pool.fetchval("SELECT user_id FROM users WHERE username=$1", username)
...
async def cancel_task(update, context):
    await delete_cmd(update); username = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool'); is_adm = await is_bot_admin(username, pool)
...
async def cancel_poll_admin(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    if not update.message.reply_to_message or not update.message.reply_to_message.poll: return await context.bot.send_message(update.effective_user.id, "❌ Reply to live poll with `/cancelpoll`.")
    try:
...
async def set_weekly_limit(update, context):
    ...
    async with pool.acquire() as conn: await conn.execute("INSERT INTO config (key, value) VALUES ('gemini_weekly_limit', $1) ON CONFLICT (key) DO UPDATE SET value=$1", str(limit))
...
async def set_bday_time(update, context):
    ...
    async with pool.acquire() as conn: await conn.execute("INSERT INTO config (key, value) VALUES ('bday_time', $1) ON CONFLICT (key) DO UPDATE SET value=$1", formatted_time)
...
async def add_bday(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    try: parts = [p.strip() for p in " ".join(context.args).split(",")]; u, b = parts[0].replace("@", "").lower(), parts[1]
    except: return await context.bot.send_message(update.effective_user.id, "❌ Format: `/addbday @user, MM/DD`")
    async with pool.acquire() as conn:
...
async def edit_bday(update, context):
    ...
    if not await is_bot_admin(update.effective_user.username, pool): return
    try: parts = [p.strip() for p in " ".join(context.args).split(",")]; u, b = parts[0].replace("@", "").lower(), parts[1]
    except: return await context.bot.send_message(update.effective_user.id, "❌ Format: `/editbday @user, MM/DD`")
    async with pool.acquire() as conn: res = await conn.execute('UPDATE birthdays SET bday=$1 WHERE lower(username)=$2', b, u)
...
async def list_bdays(update, context):
    ...
    await context.bot.send_message(update.effective_user.id, "🎂 **Birthdays Matrix**\n" + "\n".join([f"• @{x['username']}: {x['bday']}" for x in b]))

async def addlib_batch(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    lines = update.message.text.split('\n'); items = [lines[0].split(' ', 1)[1].strip()] if len(lines[0].split(' ', 1)) > 1 else []; items.extend([l.strip() for l in lines[1:] if l.strip()])
    async with pool.acquire() as conn:
        for item in items:
            try:
                is_p = False; raw = item
                if raw.lower().endswith(", private"): is_p = True; raw = raw[:-9].strip()
                p = [x.strip() for x in raw.split(",", 1)]; await conn.execute('INSERT INTO library (name, content, added_by, is_private) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING', p[0].lower(), p[1], 'AdminBatch', is_p)
            except: pass
...
async def addbday_batch(update, context):
    ...
    async with pool.acquire() as conn:
        for item in items:
            try: p = [x.strip() for x in item.split(",")]; await conn.execute('INSERT INTO birthdays (username, bday) VALUES ($1, $2) ON CONFLICT DO NOTHING', p[0].replace("@", "").lower(), p[1])
            except: pass
...
async def analyze_feedback(update, context):
    ...
    except ValueError: return await context.bot.send_message(update.effective_user.id, "❌ Date format error. Use MM/DD/YYYY (e.g., 06/15/2026)")
    except Exception as e: return await context.bot.send_message(update.effective_user.id, f"❌ Database error: {e}")
...
async def all_time_feedback(update, context):
    ...
    msg = "📋 🗄️ **Historical Archive Feedback Feed**\n\n" + "".join([f"• `[{r['created_at'].astimezone(WIB).strftime('%m/%d/%Y')}]` @{r['username']}: {r['report']}\n" for r in recs])
    await send_md(context, update.effective_user.id, msg)
...
async def check_group_id(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    if update.effective_chat.type in ['group', 'supergroup']: await context.bot.send_message(update.effective_chat.id, f"✅ 📌 **Group Info**\nTitle: `{update.effective_chat.title}`\nID: `{update.effective_chat.id}`", parse_mode="Markdown")
    else:
...
async def force_back(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    ...
    try:
        async with pool.acquire() as conn:
            ...
            if not status: return await context.bot.send_message(update.effective_user.id, f"❌ @{target} is Available 🟢.")
        ...
        if uid:
            try: await context.bot.send_message(uid, f"⚠️ Admin removed Away status.\n\n{msg}", parse_mode="Markdown")
            except: pass
...
async def group_tasks(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    now = datetime.datetime.now(WIB)
    try:
        async with pool.acquire() as conn: tasks = await conn.fetch("SELECT id, assignee, assigned_by, task_desc, deadline FROM tasks WHERE status='Pending' ORDER BY deadline")
        if not tasks: msg = "✅ 🎉 Zero pending tasks."
        else:
...
async def super_reset_req(update, context):
    await delete_cmd(update); if not await is_super(update.effective_user.username): return
...
async def super_callback(update, context):
    ...
    async with pool.acquire() as conn:
        if act == "addadmin":
            await conn.execute('INSERT INTO bot_admins (username) VALUES ($1) ON CONFLICT DO NOTHING', t); await q.edit_message_text(f"Promoted @{t}.")
        elif act == "deladmin":
            await conn.execute('DELETE FROM bot_admins WHERE username=$1', t); await q.edit_message_text(f"Demoted @{t}.")
        elif act == "removemember":
            await conn.execute('DELETE FROM kudos WHERE username=$1', t); await conn.execute('DELETE FROM birthdays WHERE username=$1', t); await q.edit_message_text(f"Offboarded @{t}.")
        elif act == "reset":
...
async def pause_bot(update, context):
    await delete_cmd(update); if not await is_super(update.effective_user.username or str(update.effective_user.id)): return
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: await conn.execute("INSERT INTO config (key, value) VALUES ('maintenance_mode', 'true') ON CONFLICT (key) DO UPDATE SET value='true'")
...
async def restart_bot(update, context):
    await delete_cmd(update); if not await is_super(update.effective_user.username or str(update.effective_user.id)): return
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: await conn.execute("INSERT INTO config (key, value) VALUES ('maintenance_mode', 'false') ON CONFLICT (key) DO UPDATE SET value='false'")
    await context.bot.send_message(update.effective_user.id, "▶️ **System Restarted.**", parse_mode="Markdown")

async def process_schedules(context):
    pool = context.bot_data.get('db_pool'); if not pool: return
    now = datetime.datetime.now(WIB)
    async with pool.acquire() as conn:
        schedules = await conn.fetch("SELECT * FROM scheduled_announcements")
        for s in schedules:
            should_run = False; f = s['frequency']; t_str = s['run_time']
            try:
                if f == 'once':
                    run_dt = WIB.localize(datetime.datetime.strptime(t_str, "%m/%d/%Y %H.%M"))
                    if now >= run_dt and not s['last_run']: should_run = True
                elif f == 'daily':
                    h, m = map(int, t_str.split('.')); if now.hour == h and now.minute == m:
                        if not s['last_run'] or s['last_run'].date() < now.date(): should_run = True
                elif f == 'weekly':
                    day, tm = t_str.split(' '); h, m = map(int, tm.split('.')); if now.weekday() == int(day) and now.hour == h and now.minute == m:
                        if not s['last_run'] or (now - s['last_run'].astimezone(WIB)).days >= 6: should_run = True
            except: continue
            if should_run:
                msg = f"📢 **Scheduled Announcement**\n\n{s['message']}"
                if s['mention']:
                    users = await conn.fetch("SELECT username FROM users WHERE username IS NOT NULL"); msg += f"\n\n👥 " + " ".join([f"@{u['username']}" for u in users])
...
async def schedule_announcement(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
    try:
        parts = [p.strip() for p in " ".join(context.args).split(",", 4)]; chat_id = parts[0].lower(); freq = parts[1].lower(); t_str = parts[2]
        mention = parts[3].lower() in ['mention', 'yes', 'y', 'true']; msg = parts[4]
        if freq == 'once': datetime.datetime.strptime(t_str, "%m/%d/%Y %H.%M")
        elif freq == 'daily': h, m = map(int, t_str.split('.')); assert 0<=h<=23 and 0<=m<=59
        elif freq == 'weekly': d, tm = t_str.split(' '); h, m = map(int, tm.split('.')); assert 0<=int(d)<=6 and 0<=h<=23 and 0<=m<=59
    ...
    async with pool.acquire() as conn: await conn.execute("INSERT INTO scheduled_announcements (chat_id, frequency, run_time, mention, message, created_by) VALUES ($1, $2, $3, $4, $5, $6)", chat_id, freq, t_str, mention, msg, update.effective_user.username)
    await update.message.reply_text("✅ Announcement scheduled successfully!")

async def list_schedules(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
...
async def del_schedule(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(update.effective_user.username, pool): return
