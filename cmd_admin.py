...
async def auto_return_away(context):
    pool = context.bot_data.get('db_pool'); username = context.job.data['username']
...
async def cancel_task(update, context):
    await delete_cmd(update); pool = context.bot_data.get('db_pool'); username = update.effective_user.username or str(update.effective_user.id)
...
async def force_back(update, context):
    ...
    try:
        ...
        if uid: await context.bot.send_message(uid, f"⚠️ Admin removed Away status.\n\n{msg}", parse_mode="Markdown")
