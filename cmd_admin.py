...
async def config_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        new_val = int(val) + increments.get(act, 1) if val else increments.get(act, 1)
        if new_val > 100:
            new_val = 1
...
async def poll_reminder(context: ContextTypes.DEFAULT_TYPE):
    try:
        await context.bot.send_message(context.job.data['chat_id'], f"⏳ **Poll Reminder:** The poll '{context.job.data['q']}' is ending in 15 minutes!", parse_mode="Markdown")
    except Exception:
        pass
