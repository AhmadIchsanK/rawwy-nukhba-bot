...
async def security_track_chats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status in ['member', 'administrator']:
        ...
        if not await is_bot_admin(inviter, pool):
            try:
                await context.bot.send_message(chat.id, "❌ **Access Denied.** Leave chat.")
        ...
        for uid in admin_ids:
            try: await context.bot.send_message(uid, f"✅ **Bot Joined Group**: {chat.title} (`{chat.id}`)", parse_mode="Markdown")
            except: pass
    elif status in ['left', 'kicked']:
        async with pool.acquire() as conn:
            await conn.execute('DELETE FROM active_groups WHERE chat_id=$1', chat.id)
        for uid in admin_ids:
            try: await context.bot.send_message(uid, f"⚠️ **Bot Left Group**: {chat.title} (`{chat.id}`)", parse_mode="Markdown")
            except: pass

async def submit_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = " ".join(context.args)
    if not text: return await update.message.reply_text("❌ Please type: `/feedback [explain issue or feature request]`")
    username = update.effective_user.username or str(update.effective_user.id)
    pool = context.bot_data.get('db_pool')
    try:
        async with pool.acquire() as conn: 
            await conn.execute("INSERT INTO bug_reports (username, report) VALUES ($1, $2)", username, text)
        await log_action(pool, update.effective_user.id, update.effective_chat.id, "Feedback", "Success", f"Feedback from @{username}")
...
async def ask_gemini(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if not is_adm:
                ...
                if quota <= 0: return await update.message.reply_text(f"❌ Gemini quota depleted ({limit}/{limit}).")
    ...
    temp = await update.message.reply_text("⏳ Gemini is thinking...")
    try:
        ...
        if len(reply) > 500 and update.effective_chat.type != "private":
            try:
                ...
                await temp.edit_text(f"✅ Response over 500 characters, routed securely to DMs.\n\n{quota_msg}", parse_mode="Markdown")
            except:
                await temp.edit_text("❌ Cannot route to DMs. Open a private chat with me first.")
                if not is_adm:
        ...
        else: await temp.edit_text(f"🤖 **Gemini:** {reply}\n\n{quota_msg}", parse_mode="Markdown")
    except Exception as e:
