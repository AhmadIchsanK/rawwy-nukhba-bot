...
async def what_did_i_miss(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not history:
        ...
        except Exception: await update.message.reply_text("❌ Please start a DM with me first to receive your recap.")
        return
...
async def submit_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private":
        ...
        except Exception: await context.bot.send_message(update.effective_chat.id, "❌ Please DM me first to submit feedback securely.")
        return
        
    text = " ".join(context.args)
    if not text:
        return await context.bot.send_message(update.effective_user.id, "❌ Usage: `/feedback [explain issue or request here]`", parse_mode="Markdown")
...
async def process_feedback_submission(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Confirm & Submit", callback_data="fb_submit")], [InlineKeyboardButton("✏️ Edit Once", callback_data="fb_edit")]])
...
async def feedback_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "fb_edit":
        ...
        if feedback_channel:
            try: await context.bot.send_message(int(feedback_channel), f"💡 **New Feedback from @{username}:**\n\n{text}")
            except Exception: pass
...
async def process_gemini_request(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str, is_bot_query: bool = False):
    ...
    try:
        ...
        if is_bot_query:
            ...
            system_prompt = (
                "You are Nukhba Manager, an advanced Telegram bot. You have absolute knowledge of all your capabilities.\n\n"
                "== COMMAND REFERENCE ==\n" + cmd_list + "\n\n== LIVE SYSTEM LIMITS ==\n"
            ...
            if is_adm:
                system_prompt += (
                    "If the admin asks to configure or change a hidden setting, output a JSON block exactly like this:\n"
                    f"{bt}json\n"
        ...
        response = await asyncio.wait_for(asyncio.to_thread(client.models.generate_content, model='gemini-2.5-flash', contents=system_prompt), timeout=120.0)
        reply = response.text
        if len(reply) > 3000: reply = reply[:2997] + "..."
        ...
        if is_bot_query and is_adm:
            ...
            if match:
                try:
                    ...
                    async with pool.acquire() as conn:
                        for c in configs: await conn.execute("INSERT INTO config (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value=$2", c['key'], str(c['value']))
        ...
        if len(final_reply) > dm_len and update.effective_chat.type != "private":
            try:
                try: await context.bot.send_message(update.effective_user.id, f"{prefix}{final_reply}", parse_mode="Markdown")
                except Exception: await context.bot.send_message(update.effective_user.id, f"{prefix}{final_reply}")
                try: await temp.edit_text(f"✅ It's a bit long, so I sent the answer to your DMs!\n\n{quota_msg}", parse_mode="Markdown")
                except Exception: await temp.edit_text(f"✅ It's a bit long, so I sent the answer to your DMs!\n\n{quota_msg}")
            except Exception:
            ...
            try: await temp.edit_text(f"{inline_prefix}{final_reply}\n\n{quota_msg}", parse_mode="Markdown")
            except Exception: await temp.edit_text(f"{inline_prefix}{final_reply}\n\n{quota_msg}")
...
async def send_md_chunks(bot, chat_id, text, prefix="", suffix=""):
    ...
    if len(full) <= limit:
        ...
        except Exception: await bot.send_message(chat_id, full)
        return
    ...
    for chunk in chunks:
        if chunk.strip():
            ...
            except Exception: await bot.send_message(chat_id, chunk)
...
async def flush_chat_buffer(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not buffer and stats_uses == 0: return
    ...
    try:
        async with pool.acquire() as conn:
            ...
            if stats_uses > 0:
                ...
                await conn.execute("INSERT INTO bot_stats (date, uses, errors) VALUES (CURRENT_DATE, $1, 0) ON CONFLICT (date) DO UPDATE SET uses = bot_stats.uses + $1", stats_uses)
    except Exception as e:
