...
# --- 1/ EVENTS ---
async def create_event(update, context):
    ...
    except Exception as e: return await update.message.reply_text("❌ Format: `/newevent Title , MM/DD/YYYY HH.MM , RemMins`", parse_mode="Markdown")
    ...
    async with pool.acquire() as conn: e_id = await conn.fetchval('INSERT INTO events (title, event_time, created_by, chat_id, msg_id) VALUES ($1, $2, $3, $4, $5) RETURNING id', title, e_time, username, update.effective_chat.id, msg.message_id)
...
async def edit_event(update, context):
    username = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
    try:
        parts = " ".join(context.args).split(",", 1); e_id = int(parts[0].strip())
        title, time_str, rem_str = [p.strip() for p in parts[1].rsplit(",", 2)]
        e_time = WIB.localize(datetime.datetime.strptime(time_str, "%m/%d/%Y %H.%M")); rem = int(rem_str)
    except: return await update.message.reply_text("❌ Format: `/editevent ID , Title , MM/DD/YYYY HH.MM , RemMins`", parse_mode="Markdown")
    async with pool.acquire() as conn: 
        ...
        if ev['created_by'] != username and not await is_bot_admin(username, pool): return await update.message.reply_text("❌ Unauthorized.")
...
async def rsvp_callback(update, context):
    q = update.callback_query
    if q.data.startswith("rsvp_temp"): return await q.answer("Initializing...")
    _, e_id, status = q.data.split("_"); username = q.from_user.username or str(q.from_user.id); pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
    ...
    await q.edit_message_text(text, reply_markup=q.message.reply_markup, parse_mode="Markdown"); await q.answer(status)
...
async def create_poll(update, context):
    if update.effective_chat.type == "private": return await update.message.reply_text("❌ Must be in a group.")
    try: parts = [p.strip() for p in " ".join(context.args).split(",") if p.strip()]; q = parts[0]; opts = parts[1:11]
    except: return await update.message.reply_text("❌ Format: `/poll Question , Opt1 , Opt2`", parse_mode="Markdown")
    if len(opts) < 2: return await update.message.reply_text("❌ Need at least 2 options.")
...
async def poll_callback(update, context):
    q = update.callback_query; _, pid, act = q.data.split("_"); pid = int(pid)
    draft = context.chat_data.get(f"poll_{pid}")
    if not draft: await q.message.delete(); return await q.answer("Expired.", show_alert=True)
    if update.effective_user.id != draft['owner']: return await q.answer("❌ Unauthorized.", show_alert=True)
    if act == "anon": draft['anon'] = not draft['anon']
    elif act == "multi":
        if draft['quiz_idx'] >= 0: return await q.answer("❌ Disabled in Quiz mode!", show_alert=True)
        ...
        cycles = [1, 6, 12, 24, 48, 72]; draft['hours'] = cycles[(cycles.index(draft['hours']) + 1) % 6] if draft['hours'] in cycles else 24
    elif act == "cancel": del context.chat_data[f"poll_{pid}"]; await q.message.delete(); return await q.answer("Cancelled.")
    elif act == "send":
        pool = context.bot_data.get('db_pool'); dur = draft['hours'] * 3600
        async with pool.acquire() as conn:
        ...
        try:
            ...
            async with pool.acquire() as conn: await conn.execute("INSERT INTO active_polls (chat_id, user_id, end_time) VALUES ($1, $2, $3) ON CONFLICT (chat_id, user_id) DO UPDATE SET end_time=$3", update.effective_chat.id, draft['owner'], end_time)
            if dur > 900:
            ...
            del context.chat_data[f"poll_{pid}"]; await q.message.delete(); return await q.answer("Launched!")
        except Exception as e: return await q.answer(f"❌ Error: {e}", show_alert=True)
    await q.edit_message_reply_markup(reply_markup=get_poll_kb(draft, pid))

# --- 3/ STARS ---
async def give_thanks(update, context):
    if not update.message.reply_to_message: return await update.message.reply_text("❌ Reply to a message to give a Star!")
    giver = update.effective_user.username or str(update.effective_user.id); receiver_user = update.message.reply_to_message.from_user; receiver = receiver_user.username or str(receiver_user.id)
    if receiver_user.is_bot or giver == receiver: return await update.message.reply_text("❌ Cannot star bots or yourself.")
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
        ...
        if q <= 0: return await update.message.reply_text("❌ Quota empty until Monday.")
...
async def my_quota(update, context):
    user = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
...
async def my_star(update, context):
    user = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
...
async def total_star(update, context):
    user = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
...
# --- 4/ LIBRARY ---
async def add_lib(update, context):
    raw = " ".join(context.args)
    if not raw: return await update.message.reply_text("❌ `/addlib Name , Content`", parse_mode="Markdown")
    is_p = False
    if raw.lower().endswith(", private"): is_p = True; raw = raw[:-9].strip(); await delete_cmd(update)
    try: parts = [p.strip() for p in raw.split(",", 1)]; name = parts[0].lower(); content = parts[1]
    except: return await update.message.reply_text("❌ Format error.")
    pool = context.bot_data.get('db_pool'); username = update.effective_user.username or str(update.effective_user.id)
    async with pool.acquire() as conn:
        if await conn.fetchval('SELECT name FROM library WHERE name=$1', name): return await update.message.reply_text("❌ Name taken.")
        await conn.execute('INSERT INTO library (name, content, added_by, is_private) VALUES ($1, $2, $3, $4)', name, content, username, is_p)
    try: await context.bot.send_message(update.effective_user.id if is_p else update.effective_chat.id, f"✅ Asset **'{name}'** added! {'🔒' if is_p else ''}", parse_mode="Markdown")
    except: pass

async def edit_lib(update, context):
    username = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
    try: parts = [p.strip() for p in " ".join(context.args).split(",", 1)]; name = parts[0].lower(); content = parts[1]
    except: return await update.message.reply_text("❌ Format error.")
    async with pool.acquire() as conn:
        ...
        if asset['added_by'] != username and not await is_bot_admin(username, pool): return await update.message.reply_text("❌ Unauthorized.")
...
async def get_lib(update, context):
    try: name = context.args[0].lower().strip()
    except: return await update.message.reply_text("❌ `/getlib Name`", parse_mode="Markdown")
    username = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
...
async def assign_task(update, context):
    pool = context.bot_data.get('db_pool')
    try: parts = [p.strip() for p in " ".join(context.args).rsplit(",", 2)]; a = parts[0].replace("@", ""); m = int(parts[1]); d = parts[2]
    except: return await update.message.reply_text("❌ `/assign @user , Mins , Desc`", parse_mode="Markdown")
    assigner = update.effective_user.username or str(update.effective_user.id)
    if a.lower() == assigner.lower(): return await update.message.reply_text("❌ Cannot self-assign.")
    if m < 60 or m > 480: return await update.message.reply_text("❌ Must be 60-480m.")
    dl = datetime.datetime.now(WIB) + datetime.timedelta(minutes=m)
    async with pool.acquire() as conn: 
        if not await is_bot_admin(assigner, pool) and await conn.fetchval("SELECT COUNT(*) FROM tasks WHERE assigned_by=$1 AND status='Pending'", assigner) >= 4: return await update.message.reply_text("❌ Max 4 active assigned tasks allowed.")
...
async def complete_task(update, context):
    ...
    except: return await update.message.reply_text("❌ `/complete ID`", parse_mode="Markdown")
    async with pool.acquire() as conn:
        task = await conn.fetchrow('SELECT assignee, status FROM tasks WHERE id=$1', t_id)
        if not task: return await update.message.reply_text("❌ Not found.")
        if task['status'] == 'Completed': return await update.message.reply_text("❌ Already done.")
        if task['assignee'] != username: return await update.message.reply_text("❌ Only assignee can complete.")
...
async def my_tasks(update, context):
    username = update.effective_user.username or str(update.effective_user.id); now = datetime.datetime.now(WIB); pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: tasks = await conn.fetch("SELECT id, task_desc, deadline FROM tasks WHERE status='Pending' AND assignee=$1 ORDER BY deadline", username)
...
async def process_return(username, pool, bot):
    async with pool.acquire() as conn:
        ...
        await conn.execute('DELETE FROM away_status WHERE username=$1', username); await conn.execute('DELETE FROM away_mentions WHERE away_username=$1', username)
    msg = f"✅ 🎉 Welcome back, @{username}! Status: 🟢 Available.\n\n"
    if mentions: msg += "Away Mentions:\n\n" + "".join([f"🔹 [{m['created_at'].astimezone(WIB).strftime('%m/%d %H:%M')}] in {m['chat_title']}\n@{m['mentioner']}: \"{m['message']}\"\n\n" for m in mentions])
    else: msg += "No mentions while away."
    return msg

async def set_away(update, context):
    username = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
    try:
        async with pool.acquire() as conn:
            status = await conn.fetchrow("SELECT end_time FROM away_status WHERE username=$1", username)
            if status: return await update.message.reply_text(f"❌ Already Away until {status['end_time'].astimezone(WIB).strftime('%m/%d %H:%M')}.")
        parts = [p.strip() for p in " ".join(context.args).rsplit(",", 1)]; reason, end_time = parts[0], WIB.localize(datetime.datetime.strptime(parts[1], "%m/%d/%Y %H.%M"))
        if end_time < datetime.datetime.now(WIB): return await update.message.reply_text("❌ Time is in the past.")
    except: return await update.message.reply_text("❌ `/away Reason , MM/DD/YYYY HH.MM`", parse_mode="Markdown")
    async with pool.acquire() as conn: await conn.execute('INSERT INTO away_status (username, reason, end_time) VALUES ($1, $2, $3)', username, reason, end_time)
...
async def set_back(update, context): 
    await delete_cmd(update); username = update.effective_user.username or str(update.effective_user.id); pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
        if not await conn.fetchrow('SELECT * FROM away_status WHERE username=$1', username): return
