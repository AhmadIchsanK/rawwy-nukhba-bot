...
async def create_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    import cmd_admin
    context.job_queue.run_once(cmd_admin.event_reminder, when=e_time - datetime.timedelta(minutes=rem), chat_id=update.effective_chat.id, data={"id": e_id, "title": title}, name=f"event_rem_{e_id}")
    context.job_queue.run_once(cmd_admin.unpin_event, when=e_time, data={"chat_id": update.effective_chat.id, "msg_id": msg.message_id}, name=f"event_unpin_{e_id}")
...
# --- 2/ POLLS (Database Patched) ---
async def create_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type == "private": return await update.message.reply_text("❌ Group only.")
    try:
        parts = [p.strip() for p in " ".join(context.args).split(",") if p.strip()]
        q = parts[0]; opts = parts[1:11]
    except: return await update.message.reply_text("❌ `/poll Question , Opt1 , Opt2`", parse_mode="Markdown")
    ...
    await update.message.reply_text(f"📊 **Poll Setup**\n\n**Q:** {q}\n\n{opts_str}", reply_markup=await get_poll_kb(pid, pool), parse_mode="Markdown")

async def get_poll_kb(pid, pool):
    async with pool.acquire() as conn: d = await conn.fetchrow("SELECT * FROM poll_drafts WHERE pid=$1", pid)
...
async def poll_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    _, pid_str, act = q.data.split("_"); pid = int(pid_str)
    pool = context.bot_data.get('db_pool')
    ...
    async with pool.acquire() as conn:
        ...
        if not draft: 
            ...
            return await q.answer("Draft expired (Server Restarted).", show_alert=True)
        if update.effective_user.id != draft['owner']: return await q.answer("Unauthorized.", show_alert=True)
        
        if act == "anon": await conn.execute("UPDATE poll_drafts SET anon = NOT anon WHERE pid=$1", pid)
        elif act == "multi": await conn.execute("UPDATE poll_drafts SET multi = NOT multi WHERE pid=$1", pid)
        elif act == "quiz":
            ...
            await q.message.delete(); return await q.answer("Cancelled.")
        elif act == "send":
            opts = json.loads(draft['opts']); dur = draft['hours'] * 3600
            try:
                ...
                await conn.execute("DELETE FROM poll_drafts WHERE pid=$1", pid)
                await q.message.delete(); return await q.answer("Launched!")
            except Exception as e: return await q.answer(f"Error: {e}", show_alert=True)
            
    await q.edit_message_reply_markup(reply_markup=await get_poll_kb(pid, pool))
...
async def assign_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn: 
        mt = int(await conn.fetchval("SELECT value FROM config WHERE key='max_tasks'") or '4')
        if not await is_bot_admin(assigner, pool) and await conn.fetchval("SELECT COUNT(*) FROM tasks WHERE assigned_by=$1 AND status='Pending'", assigner) >= mt: 
            return await update.message.reply_text(f"❌ Max {mt} active assigned tasks allowed.")
    ...
    import cmd_admin
    context.job_queue.run_once(cmd_admin.task_reminder, when=dl - datetime.timedelta(minutes=10), data={"assignee": a, "assigner": assigner, "id": t_id, "desc": d}, chat_id=update.effective_chat.id)
...
async def set_away(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reason = parts[0]; end_time = WIB.localize(datetime.datetime.strptime(parts[1], "%m/%d/%Y %H.%M"))
    ...
    import cmd_admin
    context.job_queue.run_once(cmd_admin.auto_return_away, when=end_time, data={"username": username, "chat_id": update.effective_chat.id}, name=f"away_{username}")
