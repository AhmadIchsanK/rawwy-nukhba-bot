...
async def init_db(app: Application):
    ...
    async with app.bot_data['db_pool'].acquire() as conn:
        ...
        # Upgrade existing library table safely if it exists
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.effective_chat.type != "private": await update.message.reply_text("📬 I've slid into your DMs with the manual!")
    except:
        if update.effective_chat.type != "private": await update.message.reply_text("❌ Oops! I can't DM you. Please message me directly first!")
...
async def add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: return await context.bot.send_message(update.effective_user.id, "❌ Use `/addadmin @user`")
    pool = context.bot_data.get('db_pool')
...
async def del_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_super(update.effective_user.username): return
    try: target = context.args[0].replace("@", "").lower()
    except: return await context.bot.send_message(update.effective_user.id, "❌ Use `/deladmin @user`")
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: res = await conn.execute('DELETE FROM bot_admins WHERE username=$1', target)
    await context.bot.send_message(update.effective_user.id, f"🗑️ @{target} removed from Admins." if res != "DELETE 0" else "❌ User not an admin.")
...
async def remove_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: return await context.bot.send_message(update.effective_user.id, "❌ Use `/removemember @user`")
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
...
async def announce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "," not in text: return await context.bot.send_message(update.effective_user.id, "❌ `/announce All , Message`")
    try: target, msg = [p.strip() for p in text.split(",", 1)]
    except: return await context.bot.send_message(update.effective_user.id, "❌ Missing message.")
    async with pool.acquire() as conn:
    ...
    await context.bot.send_message(update.effective_user.id, f"✅ Announcement `{a_id}` sent to {sent} groups.")
...
# --- 4/ RAWWY LIBRARY (Private Features Added) ---
async def add_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = " ".join(context.args)
    if "," not in text: return await update.message.reply_text("❌ `/addlib Name , Link , [private]`")
    try:
        ...
        if len(parts) >= 3 and parts[2].lower() == 'private':
            is_private = True
            await delete_cmd(update) # Delete trigger from public chat to hide content
    ...
    try: await context.bot.send_message(target_chat, f"✅ Saved **{name}**! {'🔒 (Private)' if is_private else ''}", parse_mode="Markdown")
    except: pass # Fails if DM isn't started, but code saved it safely

async def get_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: return await update.message.reply_text("❌ `/getlib Name`")
    ...
    if r['is_private']:
        await delete_cmd(update) # Hide from group
        ...
        except: await update.message.reply_text("❌ Start a DM with me to view your private files.")
    else:
...
async def del_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: return await update.message.reply_text("❌ `/dellib Name`")
    name = " ".join(context.args).strip().lower()
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
        ...
        if r['is_private']:
            await delete_cmd(update)
            if r['added_by'] != update.effective_user.username: return await context.bot.send_message(update.effective_user.id, "❌ Not your private file.")
    ...
    try: await context.bot.send_message(target, "🗑️ Deleted.")
    except: pass

# --- 6/ AWAY SYSTEM ---
async def set_away(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = " ".join(context.args)
    if "," not in text: return await update.message.reply_text("❌ Format: `/away Reason , MM/DD/YYYY HH.MM`")
    try:
        reason, time_str = [p.strip() for p in text.split(",", 1)]
        end_time = WIB.localize(datetime.datetime.strptime(time_str, "%m/%d/%Y %H.%M"))
        if end_time < datetime.datetime.now(WIB): return await update.message.reply_text("❌ Return time cannot be in the past.")
    except: return await update.message.reply_text("❌ Format: `MM/DD/YYYY HH.MM`")
...
async def process_return(username, bot, chat_id=None):
    ...
    msg = "👋 Welcome back! Here is what you missed:\n\n" + ("No mentions." if not mentions else "".join([f"🔹 **@{m['mentioner']}** in *{m['chat_title']}*:\n\"{m['message']}\"\n\n" for m in mentions]))
    try: 
        if chat_id: await bot.send_message(chat_id, f"Welcome back @{username}! Removed your Away status.")
    except: pass
...
async def give_thanks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if giver == receiver or update.message.reply_to_message.from_user.is_bot: return await update.message.reply_text("❌ Bots/Yourself cannot get stars.")
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
        ...
        if q <= 0: return await update.message.reply_text("❌ You are out of RAWWY Stars!")
...
async def check_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_bot_admin(update.effective_user.username, pool): return
    try: target = context.args[0].replace("@", "")
    except: return await context.bot.send_message(update.effective_user.id, "❌ `/checkstars all` OR `/checkstars @user`")
    async with pool.acquire() as conn:
        if target.lower() == 'all':
...
async def admin_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_bot_admin(update.effective_user.username, pool): return
    try:
        t, act, amt = [p.strip() for p in " ".join(context.args).split(",", 2)]; t = t.replace("@", "")
    except: return await context.bot.send_message(update.effective_user.id, "❌ `/admin_stars @user , set_quota/add_total/sub_total , amount`")
    async with pool.acquire() as conn:
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: recs = await conn.fetch('SELECT username, monthly_points FROM kudos WHERE monthly_points > 0 ORDER BY monthly_points DESC LIMIT 5')
    if not recs: return await update.message.reply_text("📊 **Leaderboard**\nNo points given!")
...
async def create_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: return await update.message.reply_text("❌ `/newevent Title , MM/DD/YYYY HH.MM , RemMins`")
    pool = context.bot_data.get('db_pool')
...
async def edit_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: e_id, title, t_str, rem = [p.strip() for p in " ".join(context.args).split(",", 3)]; e_time = WIB.localize(datetime.datetime.strptime(t_str, "%m/%d/%Y %H.%M"))
    except: return await update.message.reply_text("❌ `/editevent ID , Title , MM/DD/YYYY HH.MM , RemMins`")
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: await conn.execute('UPDATE events SET title=$1, event_time=$2 WHERE id=$3', title, e_time, int(e_id))
...
async def list_events(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📅 **Upcoming**\n" + "\n".join([f"🔹 **{e['title']}**\n🕒 {e['event_time'].astimezone(WIB).strftime('%m/%d/%Y %H:%M')} WIB" for e in events]) if events else "No events.", parse_mode="Markdown")
...
async def rsvp_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not event: return await q.answer("Deleted.")
...
async def assign_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: a, m, d = [p.strip() for p in " ".join(context.args).split(",", 2)]; a = a.replace("@", ""); m = int(m)
    except: return await update.message.reply_text("❌ `/assign @user , Mins , Task`")
...
async def complete_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        if t and t['assignee'] == update.effective_user.username: await conn.execute("UPDATE tasks SET status='Completed' WHERE id=$1", t_id)
    await update.message.reply_text(f"✅ Task `{t_id}` completed.")

async def cancel_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: t_id = int(context.args[0])
    except: return await update.message.reply_text("❌ `/canceltask ID`")
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn: await conn.execute("DELETE FROM tasks WHERE id=$1", t_id)
    await update.message.reply_text("🗑️ Cancelled.")

async def my_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    now = datetime.datetime.now(WIB); pool = context.bot_data.get('db_pool')
    ...
    msg = "📋 **Your Tasks**\n" + "".join([f"🔹 `{t['id']}` | {t['task_desc']} | ⏳ {int((t['deadline'] - now).total_seconds()/60)}m left\n" for t in tasks]) if tasks else "🎉 Clear!"
    try: await context.bot.send_message(update.effective_user.id, msg, parse_mode="Markdown")
    except: pass

async def create_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: parts = [p.strip() for p in " ".join(context.args).split(",")]; dur = int(parts[1]) * 3600
    except: return await update.message.reply_text("❌ `/poll Question , Hours , Opt1 , Opt2`")
...
async def add_bday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_bot_admin(update.effective_user.username, pool): return
    try: u, b = [p.strip() for p in " ".join(context.args).split(",")]
    except: return await context.bot.send_message(update.effective_user.id, "❌ `/addbday @user , MM/DD`")
    async with pool.acquire() as conn: await conn.execute('INSERT INTO birthdays (username, bday) VALUES ($1, $2)', u.replace("@", ""), b)
...
async def list_bdays(update: Update, context: ContextTypes.DEFAULT_TYPE):
    pool = context.bot_data.get('db_pool'); if not await is_bot_admin(update.effective_user.username, pool): return
