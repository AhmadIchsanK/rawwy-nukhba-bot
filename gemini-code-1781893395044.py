...
async def init_db(app: Application):
    ...
    async with app.bot_data['db_pool'].acquire() as conn:
        ...
        # New tables for Enterprise Tracking
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🤖 Hello there! **[RW] Nukhba Manager is online and tracking.** Feel free to type `/help` to see what I can do.")
...
async def monthly_leaderboard(context: ContextTypes.DEFAULT_TYPE):
    ...
    if now.day != 1: return # Only run on the first day
...
async def add_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = " ".join(context.args)
    if not text: return await update.message.reply_text("Looks like you missed something! Format: `/addlib Name , Link/Content , [private]`", parse_mode="Markdown")
    try:
...
async def get_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: return await update.message.reply_text("What asset are you looking for? Try: `/getlib Name`")
    ...
    if r['is_private']:
        ...
        await update.message.reply_text(f"📂 **{name.title()}** (Added by {r['added_by']}):\n{r['content']}", parse_mode="Markdown")
...
async def del_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args: return await update.message.reply_text("Please provide an asset to delete: `/dellib Name`")
    name = " ".join(context.args).strip().lower()
...
async def process_return(username, bot, chat_id=None):
    ...
    try: 
        if chat_id: await bot.send_message(chat_id, msg)
    except: pass
...
async def global_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        # 1. Update Global User ID mapping & Daily Uses
