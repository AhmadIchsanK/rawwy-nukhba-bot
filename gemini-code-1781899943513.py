...
async def create_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn: 
        e_id = await conn.fetchval('INSERT INTO events (title, event_time, created_by, chat_id, msg_id) VALUES ($1, $2, $3, $4, $5) RETURNING id', title, e_time, update.effective_user.username, update.effective_chat.id, msg.message_id)
...
async def edit_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn: 
        ...
        if ev['created_by'] != update.effective_user.username and not await is_bot_admin(update.effective_user.username, pool):
...
async def admin_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        await log_action(pool, f"Admin @{update.effective_user.username} executed '{act}' to modify @{t}'s {field} by {amt}.")
...
async def add_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute('INSERT INTO library (name, content, added_by, is_private) VALUES ($1, $2, $3, $4)', name, content, update.effective_user.username, is_private)
...
async def edit_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        if asset['added_by'] != update.effective_user.username and not await is_bot_admin(update.effective_user.username, pool):
...
async def get_lib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
        r = await conn.fetchrow('SELECT content, is_private FROM library WHERE name=$1', name)
...
async def cancel_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await delete_cmd(update)
    pool = context.bot_data.get('db_pool')
    is_adm = await is_bot_admin(update.effective_user.username, pool)
    try: t_id = int([p.strip() for p in " ".join(context.args).split(",") if p.strip()][0])
    ...
    async with pool.acquire() as conn:
        ...
        if task['assigned_by'] != update.effective_user.username and not is_adm: 
...
async def process_return(username, bot, chat_id=None):
    ...
    msg = f"🎉 **Welcome back, @{username}!** You are marked as Available.\n\n"
    if mentions:
    ...
    try: 
        ...
        if chat_id: await bot.send_message(chat_id, f"Welcome back @{username}! I've DM'd you your missed notifications.")
    except: pass
...
async def add_admin(u, c): await request_super_action(u, c, "addadmin", "Promote Admin")
async def del_admin(u, c): await request_super_action(u, c, "deladmin", "Demote Admin")
async def remove_member(u, c): await request_super_action(u, c, "removemember", "Offboard User")
async def super_reset(u, c): await request_super_action(u, c, "reset", "Factory Wipe")

async def super_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    if not await is_super(q.from_user.username): return await q.answer("Unauthorized.")
...
def main():
    ...
    app.job_queue.run_daily(monthly_leaderboard, datetime.time(hour=13, minute=0, tzinfo=WIB), days=(1,))
    ...
    app.add_handler(CommandHandler("checkstars", check_quota)) # Alias
