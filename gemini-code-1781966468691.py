...
```\n"
        ...
        # Run Gemini API call in thread pool without strict wait_for timeouts
...
### Changes Applied:
1. **Removed the 120-second timeout limit**: Removed the `asyncio.wait_for` wrapper completely. The API query will run via `asyncio.to_thread` for as long as needed until Gemini responds.
2. **Robust Error Handling**: Any exception thrown by the SDK (due to bad prompts, API errors, configuration issues, or service degradation) is caught by the main `try-except Exception as e` wrapper and displayed directly as `❌ AI Error: [description]`.