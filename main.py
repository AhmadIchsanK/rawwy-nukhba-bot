...
async def cmd_away(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute('INSERT INTO away_status (user_id, username, reason, end_time) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id) DO UPDATE SET reason=$3, end_time=$4', user_id, username, reason, end_time)
...
async def cmd_birthday_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(user_id, f"✅ Birthday added for @{t_uname}.")
        await log_audit(pool, user_id, username, chat_id, "Birthday Add", f"Added {t_uname}", "Success")
    except Exception as e:
        await context.bot.send_message(user_id, "❌ Format error. Try: `/addbday @user , DD/MM`", parse_mode="Markdown")
...
async def cmd_birthday_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Failed. Use `/editbday @user, DD/MM`", parse_mode="Markdown")
...
async def cmd_admin_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_audit(pool, update.effective_user.id, username, update.effective_chat.id, "Admin Star", f"Modified {t} {field}", "Success")
...
async def cmd_addlib(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not raw_args: return await update.message.reply_text("❌ Format: `/addlib Name , Link/Content , [private]`", parse_mode="Markdown")
    username = update.effective_user.username or str(update.effective_user.id)
    pool = context.bot_data.get('db_pool')
    try:
        is_private = False
...
async def cmd_canceltask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if not task: return await update.message.reply_text("❌ Task not found.")
            if task['assigned_by'] != username and not is_adm: return await update.message.reply_text("❌ Permission denied.")
