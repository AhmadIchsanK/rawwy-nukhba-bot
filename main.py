...
async def is_bot_admin(username: str, pool) -> bool:
    ...
    async with pool.acquire() as conn:
        return bool(await conn.fetchval('SELECT 1 FROM bot_admins WHERE username=$1', username.lower()))
...
def get_admin_menu():
    return get_user_menu() + [
        ...
        BotCommand("addbday", "🛠️ Add a user birthday"), BotCommand("editbday", "🛠️ Edit a user birthday"),
        BotCommand("setbdaychannel", "🛠️ Set Group for Bdays"), BotCommand("auditlog", "🛠️ Manual diagnostic report"),
        BotCommand("systemstatus", "🛠️ Live system metrics"), BotCommand("admin_stars", "🛠️ Edit Stars"),
        BotCommand("checkquota", "🛠️ Audit Quotas"), BotCommand("grouptasks", "🛠️ View all tasks"),
...
async def init_db(app: Application):
    ...
    async with app.bot_data['db_pool'].acquire() as conn:
        await conn.execute('''CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY, username TEXT);
            ...
            CREATE TABLE IF NOT EXISTS active_groups (chat_id BIGINT PRIMARY KEY, title TEXT, member_count INT);
            CREATE TABLE IF NOT EXISTS birthdays (username TEXT PRIMARY KEY, bday TEXT);
            CREATE TABLE IF NOT EXISTS away_status (user_id BIGINT PRIMARY KEY, username TEXT, reason TEXT, end_time TIMESTAMP WITH TIME ZONE);
    ...
    logger.info("✅ Database & Scopes Configured.")
...
async def compile_audit_report(pool, start_dt, end_dt, report_date_str):
    async with pool.acquire() as conn:
        ...
        stats = await conn.fetchrow("""SELECT 
            ...
            COUNT(CASE WHEN action_type = 'Event Create' THEN 1 END) as evt_cnt,
            COUNT(CASE WHEN action_type = 'RSVP' THEN 1 END) as rsvp_cnt,
            COUNT(CASE WHEN action_type = 'Announcement' AND status = 'Success' THEN 1 END) as ann_s,
            COUNT(CASE WHEN action_type = 'Announcement' AND status = 'Failed' THEN 1 END) as ann_f,
            COUNT(CASE WHEN status = 'Failed' THEN 1 END) as err_cnt
            FROM audit_logs WHERE created_at >= $1 AND created_at <= $2""", start_dt, end_dt)
        top = await conn.fetch("SELECT username, COUNT(*) as act FROM audit_logs WHERE created_at >= $1 AND created_at <= $2 AND username IS NOT NULL GROUP BY username ORDER BY act DESC LIMIT 3", start_dt, end_dt)
    
    msg = f"🌅 **Daily Diagnostic Audit Report**\nDate: {report_date_str}\n\n**Groups:**\n• Active Groups: {g_cnt}\n\n**Users:**\n• Away Count: {stats['away_cnt']}\n• Back Count: {stats['back_cnt']}\n\n**Events:**\n• Created: {stats['evt_cnt']}\n• RSVPs: {stats['rsvp_cnt']}\n\n**Announcements:**\n• Sent: {stats['ann_s']}\n• Failed: {stats['ann_f']}\n\n**System:**\n• Errors/Warnings: {stats['err_cnt']}\n\n**Top Users:**\n"
    for i, u in enumerate(top, 1): msg += f"{i}. @{u['username']} ({u['act']} actions)\n"
    return msg

async def cron_daily_audit(context: ContextTypes.DEFAULT_TYPE):
    yesterday = (datetime.datetime.now(WIB) - datetime.timedelta(days=1)).date()
...
async def cmd_auditlog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_audit(pool, uid, uname, update.effective_chat.id, "Audit Pull", "Manual Pull", "Success")
    except Exception as e:
        await log_audit(pool, uid, uname, update.effective_chat.id, "Audit Pull", str(e), "Failed")
...
async def monthly_leaderboard(context: ContextTypes.DEFAULT_TYPE):
    ...
    async with pool.acquire() as conn:
        ...
        if top:
            msg = f"🏆 **Best star earner this month is @{top['username']}!** 🏆\nTotal **{top['monthly_points']} RAWWY Stars**. Incredible work! 🌟"
...
async def daily_bday_announcement(context: ContextTypes.DEFAULT_TYPE):
    pool = context.bot_data.get('db_pool')
    async with pool.acquire() as conn:
        users = await conn.fetch("SELECT username FROM birthdays WHERE bday=$1", datetime.datetime.now(WIB).strftime("%d/%m"))
        t_group = await conn.fetchval("SELECT value FROM config WHERE key='bday_channel'")
    if users and t_group:
        msg = "🎉🎂 **HAPPY BIRTHDAY!** 🎂🎉\nPlease join me in sending warmest wishes to:\n" + "\n".join([f"🎈 @{u['username']}" for u in users])
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await sync_menus(update, context)
    await update.message.reply_text("🚀 **Workspace Guide**\nUse the dynamic `/` menu to browse tools.", parse_mode="Markdown")
...
async def cmd_away(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(args) < 2: raise ValueError("Format")
        reason, end_time = args[0].strip(), WIB.localize(datetime.datetime.strptime(args[1].strip(), "%m/%d/%Y %H.%M"))
        if end_time < datetime.datetime.now(WIB): raise ValueError("Past time")
        async with pool.acquire() as conn: await conn.execute('INSERT INTO away_status (user_id, username, reason, end_time) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id) DO UPDATE SET reason=$3, end_time=$4', uid, uname, reason, end_time)
        await update.message.reply_text(f"⚙️ Marked Away until {end_time.strftime('%H:%M WIB')}.")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Away", reason, "Success")
    except Exception as e:
        await update.message.reply_text("❌ Format: `/away Reason, MM/DD/YYYY HH.MM`")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Away", str(e), "Failed")
...
async def cmd_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with pool.acquire() as conn:
            if await conn.execute('DELETE FROM away_status WHERE user_id=$1', uid) == "DELETE 0":
                await update.message.reply_text("✅ You are already marked as Available.")
                return await log_audit(pool, uid, uname, update.effective_chat.id, "Back", "Already Available", "Success")
            await conn.execute('DELETE FROM away_mentions WHERE away_username=$1', uname)
        await update.message.reply_text("✅ You are now marked as Available.")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Back", "Returned", "Success")
    except Exception as e: 
        await update.message.reply_text("❌ Error updating status.")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Back", str(e), "Failed")
...
async def cmd_clockin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with pool.acquire() as conn: await conn.execute('INSERT INTO attendance (user_id, status) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET status=$2, updated_at=NOW()', uid, 'Clocked In')
        ...
        await log_audit(pool, uid, uname, update.effective_chat.id, "Clock In", str(e), "Failed")

async def cmd_clockout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    pool, uid, uname = context.bot_data.get('db_pool'), update.effective_user.id, update.effective_user.username or str(update.effective_user.id)
    try:
        async with pool.acquire() as conn: await conn.execute('INSERT INTO attendance (user_id, status) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET status=$2, updated_at=NOW()', uid, 'Clocked Out')
        ...
        await log_audit(pool, uid, uname, update.effective_chat.id, "Clock Out", str(e), "Failed")

# --- BIRTHDAYS ---
async def cmd_addbday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await delete_cmd(update)
    uid, uname = update.effective_user.id, update.effective_user.username or str(update.effective_user.id)
    pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(uname, pool): return
    try:
        t_uname, bday_str = [p.strip() for p in " ".join(context.args).split(",", 1)]
        t_uname = t_uname.replace("@", "")
        datetime.datetime.strptime(bday_str, "%d/%m")
        async with pool.acquire() as conn:
            if await conn.fetchval('SELECT 1 FROM birthdays WHERE username=$1', t_uname):
                await context.bot.send_message(uid, "❌ Birthday already exists. Use `/editbday` to edit the input", parse_mode="Markdown")
                return await log_audit(pool, uid, uname, update.effective_chat.id, "Birthday Add", "Duplicate", "Failed")
        ...
        await context.bot.send_message(uid, "❌ Format error. Try: `/addbday @user, DD/MM`", parse_mode="Markdown")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Birthday Add", str(e), "Failed")

async def cmd_editbday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await delete_cmd(update)
    uid, uname = update.effective_user.id, update.effective_user.username or str(update.effective_user.id)
    pool = context.bot_data.get('db_pool')
    if not await is_bot_admin(uname, pool): return
    try:
        ...
        async with pool.acquire() as conn:
            if await conn.execute('UPDATE birthdays SET bday=$1 WHERE username=$2', bday_str, t_uname) == "UPDATE 0":
                ...
                return await log_audit(pool, uid, uname, update.effective_chat.id, "Birthday Edit", "Not found", "Failed")
        ...
        await context.bot.send_message(uid, "❌ Format error: `/editbday @user, DD/MM`", parse_mode="Markdown")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Birthday Edit", str(e), "Failed")
...
async def cmd_announcement_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not draft:
        ...
        return await log_audit(pool, uid, uname, update.effective_chat.id, "Announcement", "No draft", "Failed")
    try:
        ...
        if not groups:
            ...
            return await log_audit(pool, uid, uname, update.effective_chat.id, "Announcement", "No groups", "Failed")
        success, fail = 0, 0
        for g in groups:
            try:
                ...
                fail += 1
                await log_audit(pool, uid, uname, g['chat_id'], "Announcement", f"Failed ({g['title']}): {str(e)}", "Failed")
        if success > 0:
        ...
        await log_audit(pool, uid, uname, update.effective_chat.id, "Announcement", str(e), "Failed")
...
async def cmd_admin_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not groups:
            ...
            return await log_audit(pool, uid, uname, update.effective_chat.id, "Broadcast", "No targets", "Failed")
        success, fail = 0, 0
        for g in groups:
            try:
                ...
                fail += 1
                await log_audit(pool, uid, uname, g['chat_id'], "Broadcast", f"Failed: {str(e)}", "Failed")
        if success > 0: await update.message.reply_text(f"✅ Announcement sent successfully to {success} groups. ({fail} failed).")
        else: await update.message.reply_text("❌ Failed to send announcement.\nReason: Bot lacks permission.")
    except Exception as e:
        await update.message.reply_text("❌ Format: `/admin_broadcast [ChatID|All], Message`")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Broadcast", str(e), "Failed")
...
async def security_track_chats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status in ['member', 'administrator']:
        ...
        if not await is_bot_admin(inviter, pool):
            try: await context.bot.send_message(chat.id, "❌ **Access Denied.** Unauthorized deploy.")
            except: pass
            return await context.bot.leave_chat(chat.id)
...
async def cmd_bugreport(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = update.effective_user.username or str(update.effective_user.id)
    async with context.bot_data.get('db_pool').acquire() as conn: await conn.execute("INSERT INTO bug_reports (username, report) VALUES ($1, $2)", username, text)
    await update.message.reply_text("🐛 Bug securely filed.")
    await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, username, update.effective_chat.id, "Bug Report", "Filed bug", "Success")

# --- UTILITIES (Events, Polls, Stars, Lib, Tasks) ---
async def cmd_event_create(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pool = context.bot_data.get('db_pool')
    try:
        title, e_time_str, rem_str = [p.strip() for p in " ".join(context.args).rsplit(",", 2)]
        e_time, rem = WIB.localize(datetime.datetime.strptime(e_time_str, "%m/%d/%Y %H.%M")), int(rem_str)
        if e_time < datetime.datetime.now(WIB): return await update.message.reply_text("❌ Cannot schedule in the past.")
        msg = await update.message.reply_text(f"📅 **{title} scheduled!**\n🕒 {e_time.strftime('%b %d at %H:%M WIB')}\n\n*Attendance:*\nNo RSVPs yet.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Going", callback_data="rsvp_temp_Going"), InlineKeyboardButton("❌ Not Going", callback_data="rsvp_temp_Not Going")]]), parse_mode="Markdown")
        ...
        await log_audit(pool, update.effective_user.id, uname, update.effective_chat.id, "Event Create", str(e), "Failed")
...
async def cmd_cancelevent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await delete_cmd(update)
    uid, uname, pool = update.effective_user.id, update.effective_user.username or str(update.effective_user.id), context.bot_data.get('db_pool')
    if not await is_bot_admin(uname, pool): return
    try:
        ...
        await context.bot.send_message(uid, "❌ Usage: `/cancelevent ID`")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Event Cancel", str(e), "Failed")

async def cmd_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        parts = [p.strip() for p in " ".join(context.args).split(",")]
        await context.bot.send_poll(update.effective_chat.id, parts[0], parts[2:], is_anonymous=False, allows_multiple_answers=True, open_period=int(parts[1]) * 3600)
        await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, update.effective_user.username or str(update.effective_user.id), update.effective_chat.id, "Poll Create", "Created", "Success")
    except Exception as e:
        await update.message.reply_text("❌ Format: `/poll Question, Hours, Opt1, Opt2`")
        await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, update.effective_user.username or str(update.effective_user.id), update.effective_chat.id, "Poll Create", str(e), "Failed")
...
async def cmd_raffle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, update.effective_user.username or str(update.effective_user.id), update.effective_chat.id, "Raffle", f"Won: {winner}", "Success")
...
async def cmd_thanks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with pool.acquire() as conn:
            ...
            if q <= 0: 
                await log_audit(pool, update.effective_user.id, giver, update.effective_chat.id, "Star Send", "Depleted quota", "Failed")
        ...
        await log_audit(pool, update.effective_user.id, giver, update.effective_chat.id, "Star Send", f"To {receiver}", "Success")
    except Exception as e:
        await update.message.reply_text("❌ Error processing Star.")
        await log_audit(pool, update.effective_user.id, giver, update.effective_chat.id, "Star Send", str(e), "Failed")
...
async def cmd_admin_stars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_bot_admin(uname, pool): return
    try:
        t, field, act, amt = [p.strip() for p in " ".join(context.args).split(",", 3)]
        t = t.replace("@", "")
        col = 'monthly_points' if field.lower() == 'monthly' else 'all_time_points' if field.lower() == 'total' else 'quota'
        async with pool.acquire() as conn:
            await conn.execute(f'INSERT INTO kudos (username) VALUES ($1) ON CONFLICT DO NOTHING', t)
            if act.lower() == "set": await conn.execute(f'UPDATE kudos SET {col}=$1 WHERE username=$2', int(amt), t)
            elif act.lower() == "add": await conn.execute(f'UPDATE kudos SET {col}={col}+$1 WHERE username=$2', int(amt), t)
            elif act.lower() == "sub": await conn.execute(f'UPDATE kudos SET {col}={col}-$1 WHERE username=$2', int(amt), t)
        await log_audit(pool, uid, uname, update.effective_chat.id, "Admin Star", f"Modified {t}", "Success")
        await context.bot.send_message(uid, f"✅ Updated stars for @{t}.")
    except Exception as e:
        await context.bot.send_message(uid, "❌ Usage: `/admin_stars @user, quota/monthly/total, set/add/sub, Amt`")
        await log_audit(pool, uid, uname, update.effective_chat.id, "Admin Star", str(e), "Failed")
...
async def cmd_assign(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: 
        ...
        await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, assigner, update.effective_chat.id, "Task Assign", f"Assigned to {a}", "Success")
    except Exception as e: 
        await update.message.reply_text("❌ Format: `/assign @user, Minutes, Task description`")
        await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, assigner, update.effective_chat.id, "Task Assign", str(e), "Failed")
...
async def cmd_complete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, uname, update.effective_chat.id, "Task Complete", f"Completed {t_id}", "Success")
    except Exception as e: 
        await update.message.reply_text("❌ Format: `/complete ID`")
        await log_audit(context.bot_data.get('db_pool'), update.effective_user.id, uname, update.effective_chat.id, "Task Complete", str(e), "Failed")
...
async def rsvp_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_audit(pool, q.from_user.id, uname, q.message.chat.id, "RSVP", f"RSVP {status} for {e_id}", "Success")
...
def main():
    ...
    app.add_handler(CommandHandler("addbday", cmd_addbday))
    app.add_handler(CommandHandler("editbday", cmd_editbday))
